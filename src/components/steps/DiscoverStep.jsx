 ...
 */
const DiscoverStep = () => {
  const { send, setStep, srcCfg, tgtCfg, setSourceResources, setTargetResources } = useMigration()
  const onComplete = (src, tgt) => { setSourceResources(src); setTargetResources(tgt); setStep(2) }
  ...
  return (
    <div className="animate-fade">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
        ...
        {scanning ? (
          <Btn disabled={true} variant="primary" icon={<RefreshCw size={11} className="spin" />}>
            Scanning...
          </Btn>
        ) : srcResources ? (
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
            {['source', 'target'].map(t => (
              <button key={t} onClick={() => setActiveTab(t)} style={{
                ...
                {t === 'source' ? <Database size={11} /> : (TARGETS[tgtCfg?.platform]?.iconType === 'database' ? <Database size={11} /> : <Cloud size={11} />)}
                {t === 'source' ? `${SOURCES[srcCfg?.platform]?.name || 'Source'} Source` : `${TARGETS[tgtCfg?.platform]?.name || 'Target'} Target`}
